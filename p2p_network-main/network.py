...
class Network(Node):
        ...
        while connected:
            ...
            if msg_len:
                ...
                if commands.NODE_CON_ADDR in msg["title"]:  # CONNECT TO ME COMMAND
                    ...
                    print(f"{addr} wants to establish connection")
                ...
                if commands.ASK_RANDOM_NODE in msg["title"]:
                    print(f"{addr} ASKED RANDOM NODE ")
                ...
                if commands.ASK_NODES_TO_CONNECT in msg["title"]:
                    print(f"{addr} ASKED NODES TO CONNECT", "yellow", 2)
                ...
                if self.DISCONNECT_MSG in msg["title"]:
                    ...
                    print(f"{client_ip}:{client_port} DISCONNECTED", "red", 2)
                ...
                if "#BROADCAST" == msg["title"]:
                    ...
                    if msg["message"] == "#JOINED_IN_NETWORK":
                        ...
                        if (
                            msg_sender_ip == ""
                            or msg_sender_ip == None
                            or len(msg_sender_ip) == 0
                        ):
                        ...
                        print(
                            f"{msg['sender_ip']}:{msg['sender_port']} has joined to network",
                            "green",
                        )
    ...
    def ask_random_node(self, conn, address, port):
        ...
        ask_random_message = self.short_json_msg(
            commands.ASK_RANDOM_NODE, f"{self.SERVER_IP},{self.SERVER_PORT}"
        )
    ...
    def askNodes(self, ip, port):
        ...
        print(f"{len(nodes)} Node address recieved...", "cyan")
        print(nodes)
    ...
    def connectToNode(self, address, port):
        ...
        if self.CONN_ADDR not in self.connections:
            ...
            msg = self.short_json_msg(
                commands.NODE_CON_ADDR, f"{self.SERVER_IP},{self.SERVER_PORT}"
            )

            self.send(connection, msg)

        print(f"Connected To->{address}:{port}", "green", 2)
        return
