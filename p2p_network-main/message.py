...
class Message:
    ...
    def __init__(
        self, sender_ip=None, sender_port=None, reciever_ip="all", reciever_port=None
    ):
    ...
    def msg(self, message, title):
        ts = time.time()
        id = hashlib.sha256(
            f"{self.SENDER_IP}:{self.SENDER_PORT}{self.RECIEVER_IP}:{self.RECIEVER_PORT}:{self.message_index}{ts}{self.MESSAGE}".encode()
        ).hexdigest()
    ...
    def short_msg(self, title, message=""):
        ts = time.time()
        id = hashlib.sha256(
            f"{random.randint(0,99999)}{title}{message}{time}".encode()
        ).hexdigest()
        msg_obj = {
            "id": id,
            "title": title,
